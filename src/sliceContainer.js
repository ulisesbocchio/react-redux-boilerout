...
function getLifecycleMethods(clazz) {
    const lifecycleBehavior = Reflect.ownKeys(clazz.prototype).filter(method => LIFECYCLE_EVENTS.includes(method));
    let hoister;
...
export default function sliceContainer({ slice, actions, component }) {
    return containerClass => {
        ...
        const mapSliceStateToProps =
            containerClass.mapSliceStateToProps && containerClass.mapSliceStateToProps.bind(containerClass);
