...
describe('Connect Slice Reducers Tests', () => {
  ...
  it('checks actions get correctly injected into component', () => {
    const SillyComp = ({ message, start }) => <div id="silly" onClick={start}>{message}</div>;
    class SampleReducerClass {
      start = () => ({ message: 'hello', started: true });
    }
  ...
  it('checks component only re-render when their slice or props changed', () => {
    class SillyComp1 extends React.Component {
      render() {
        ...
        return <div><div id="silly1" onClick={start}>{message}</div><Container2 /></div>;
      }
    }
    ...
    const Container1 = connectSlice(
      { slice: 'sample1', actions: SampleActions }
    )(SillyComp1);

    const Container2 = connectSlice(
      { slice: 'sample2' }
    )(SillyComp2);
    ...
    const providerWrapper = mount(<Provider store={store}><Container1 /></Provider>);
